...
const MlsDataProcessor = () => {
  const [files, setFiles] = useState<{
    mls: File | null;
    usa: File | null;
    canada: File | null;
  }>({ mls: null, usa: null, canada: null });

  const [loading, setLoading] = useState(false);
  const [status, setStatus] = useState<'idle' | 'complete' | 'error'>('idle');
  const [error, setError] = useState<string | null>(null);
  const [downloadUrl, setDownloadUrl] = useState<string | null>(null);
  const [logs, setLogs] = useState<string[]>(["System ready. Awaiting MLS data and at least one region file..."]);
  const [timestamp, setTimestamp] = useState<string | null>(null);

  const scrollRef = useRef<HTMLDivElement>(null);
  
  // Logic: MLS is mandatory AND at least one of (USA or Canada) is mandatory
  const isReady = !!(files.mls && (files.usa || files.canada));
  ...
  const handleRunCalculation = async () => {
    ...
    setStatus('idle');
    setError(null);
    setLogs(["Connecting to MLS Engine...", "Establishing stream..."]);

    const formData = new FormData();
    formData.append('mls_data', files.mls!); // Always present if isReady is true
    
    // Conditionally append region files
    if (files.usa) formData.append('usa_data', files.usa);
    if (files.canada) formData.append('canada_data', files.canada);

    try {
      const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL?.replace(/\/$/, ""); 
      const response = await fetch(`${baseUrl}/qc/calculate_mls_final_audit`, {
        method: 'POST',
        body: formData,
      });
    ...
    <div className="flex flex-col w-full h-screen bg-[#FDFDFD] dark:bg-[#08090A] overflow-hidden text-slate-900 dark:text-slate-100">
      ...
      <header className="w-full px-6 py-3 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0B0F1A] flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold tracking-tight leading-none">MLS Data Engine</h1>
            <p className="text-[10px] text-slate-500 font-medium uppercase">Cross-Border Processor</p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col p-4 gap-4 min-h-0 overflow-hidden">
        ...
        <div className="flex flex-col gap-3 shrink-0">
          ...
          >
            <input type="file" id="mls" className="hidden" onChange={(e) => handleFileChange(e, 'mls')} />
            <div className={`p-2 rounded-lg bg-slate-50 dark:bg-slate-800 text-indigo-500 shrink-0`}>
              <Database size={20} />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex justify-between items-center">
                <p className="text-[12px] font-bold truncate">
                  {files.mls?.name || "Primary MLS Data File"}
                </p>
                <span className="text-[9px] bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded uppercase font-bold tracking-wider">
                  Required
                </span>
              </div>
              <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                {files.mls ? 'File securely attached.' : 'Upload the core MLS dataset (.csv, .xlsx)'}
              </p>
            </div>
          </label>

          {/* Regional Files Block (USA / Canada) */}
          <div className="grid grid-cols-2 gap-3 relative">
            <div className="absolute -top-2 left-4 px-2 bg-[#FDFDFD] dark:bg-[#08090A] text-[9px] text-slate-400 font-bold uppercase tracking-widest z-10">
              Region Data (Requires at least one)
            </div>
            
            {[
              { id: 'usa', label: 'USA Data', icon: MapPin, color: 'text-blue-500', activeBg: 'border-blue-500 bg-blue-50/20 dark:bg-blue-500/5' },
              { id: 'canada', label: 'Canada Data', icon: MapPin, color: 'text-rose-500', activeBg: 'border-rose-500 bg-rose-50/20 dark:bg-rose-500/5' }
            ].map((item) => (
              <label 
                key={item.id}
                htmlFor={item.id}
                className={`flex items-center gap-3 px-4 py-4 border rounded-xl cursor-pointer transition-all mt-1
                  ${files[item.id as keyof typeof files] 
                    ? item.activeBg 
                    : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm hover:border-slate-400 dark:hover:border-slate-600'}`}
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[9px] text-slate-400 font-medium uppercase tracking-tight">
                    {files[item.id as keyof typeof files] ? 'Attached' : 'Optional if other is provided'}
                  </p>
                </div>
              </label>
            ))}
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0 h-64 mt-2">
          ...
          <div className="col-span-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex flex-col items-center justify-center shadow-sm relative overflow-hidden">
             <div className="text-center mb-4">
              ...
              <p className="text-[10px] text-slate-400">Cross-region alignment</p>
            </div>
            ...
            {!isReady && !loading && (
              <p className="mt-3 text-[9px] text-amber-500 font-bold uppercase flex items-center justify-center text-center gap-1">
                <AlertCircle size={14} className="shrink-0" /> 
                Requires MLS Data + (USA or Canada)
              </p>
            )}
          </div>
            ...
            {status === 'complete' && downloadUrl && (
              <button 
                onClick={() => {
                  ...
                  link.download = `MLS_Audit_Results_${Date.now()}.xlsx`;
                  link.click();
                }}
...
export default MlsDataProcessor;