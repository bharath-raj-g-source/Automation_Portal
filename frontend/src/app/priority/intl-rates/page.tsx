...
export default function IntlRateCalculatorPage() {
  ...
  // --- UPDATED: Keeps the output file name exactly the same as the input file
  const handleDownload = () => {
    if (!downloadUrl || !files.intl) return;
    const link = document.createElement('a');
    link.href = downloadUrl;
    link.download = files.intl.name; 
    link.click();
  };
  ...
  const handleRunCalculation = async () => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/qc/calculate_intl_final_audit_2`, {
        method: 'POST',
        body: formData,
        headers: headers,
      });
    ...
    <div className="flex flex-col w-full h-screen bg-[#FDFDFD] dark:bg-[#08090A] overflow-hidden text-slate-900 dark:text-slate-100">
      ...
      <main className="flex-1 flex flex-col p-6 gap-6 min-h-0 max-w-[1600px] mx-auto w-full">
        <div className="grid grid-cols-3 gap-6 shrink-0">
          {[
            { id: 'intl', label: 'BSR Data (Worksheet)', icon: FileSpreadsheet, color: 'text-indigo-500' },
