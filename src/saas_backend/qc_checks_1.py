...
def detect_header_row(bsr_path, bsr_cols):
    ...
    raise ValueError("Could not detect header row in BSR sdfasifile.")
...
def overlap_duplicate_daybreak_check(df, bsr_cols, rules):
    ...
    title_candidates = bsr_cols.get('program_title', []) + bsr_cols.get('event', []) + bsr_cols.get('program_desc', [])
    col_title = _find_column(df, title_candidates)
